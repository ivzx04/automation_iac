...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    ALTER DEFAULT PRIVILEGES FOR ROLE nocodb_domain IN SCHEMA public
      GRANT SELECT, INSERT, UPDATE ON TABLES TO n8n_workflows;

    ALTER DEFAULT PRIVILEGES FOR ROLE n8n_workflows IN SCHEMA public
      GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO nocodb_domain;

    ALTER DEFAULT PRIVILEGES FOR ROLE automation IN SCHEMA public
      GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO nocodb_domain;
